...
for i in range(len(address)):
	if '省' in address[i][1]:
		ad = address[i][1].split('省')
		sheng.append(ad[0])
	elif '省' not in address[i][1] and '市' in address[i][1]:
		ad = address[i][1].split('市')
		shi.append(ad[0])
	elif '省' not in address[i][1] and '自治区' in address[i][1]:
		ad = address[i][1].split('自治区')
		zizhi.append(ad[0])
	elif '省' not in address[i][1] and '特别行政区' in address[i][1]:
		ad = address[i][1].split('特别行政区')
		tebie.append(ad[0])
sheng = list(set(sheng))
shi = list(set(shi))
zizhi = list(set(zizhi))
tebie = list(set(tebie))
res = {}
def resData(ad):
	aData = pd.DataFrame(address)
	for each in ad:
		resList = aData[aData[1].str.contains(each)]
		res[each] = resList.values.tolist()
	print(res)
